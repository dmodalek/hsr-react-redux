...
export const getMoviesWithoutSelection = state => {
  return getMovies(state).filter(movie => state.movies.selection.includes(movie.id) === false);
};

export const getMovies3D = state => {
  const movies = getMoviesWithoutSelection(state);
  return movies.filter(movie => movie.attribute && movie.attribute.includes('_3d'));
};

export const getMoviesImax = state => {
  const movies = getMoviesWithoutSelection(state);
  return movies.filter(movie => movie.attribute && movie.attribute.includes('imax'));
};

export const getMoviesSelection = state => {
  const movies = getMovies(state);
  return movies.filter(movie => state.movies.selection.includes(movie.id));
};
